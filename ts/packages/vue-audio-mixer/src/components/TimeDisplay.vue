<template>
  ...
  <div class="vue-audio-mixer-timer">
    <span class="vue-audio-mixer-progress-time" :class="{ 'vue-audio-mixer-show-total-time': mixerVars.show_total_time }">
      ...
      <span v-if="showMins" class="vue-audio-mixer-timer-number">{{ progressFormatted[0] }}</span><span
        v-if="showMins">:</span><span class="vue-audio-mixer-timer-number">{{ progressFormatted[1] }}</span>:<span
        class="vue-audio-mixer-timer-number">{{ progressFormatted[2] }}</span>
