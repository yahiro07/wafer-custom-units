...
class App {
  ...
  _createDebugPanel() {
    ...
    const bloomFolder = this.pane.addFolder({
      title: "Bloom effect Options",
      expanded: false,
    });

    bloomFolder.addInput(this.bloomPass, "enabled", { label: "Enabled" });
    bloomFolder.addInput(this.bloomPass, "strength", {
      label: "Strength",
      min: 0,
      max: 3,
    });
    bloomFolder.addInput(this.bloomPass, "threshold", {
      label: "Threshold",
      min: 0,
      max: 1,
    });
    bloomFolder.addInput(this.bloomPass, "radius", {
      label: "Radius",
      min: 0,
      max: 1,
    });
     ...
     * Afterimage
     */
    const afterimageFolder = this.pane.addFolder({
      title: "Afterimage effect options",
      expanded: false,
    });

    afterimageFolder.addInput(this.afterimagePass, "enabled", {
      label: "Enabled",
    });
    afterimageFolder.addInput(this.afterimagePass.uniforms.damp, "value", {
      label: "Damp",
      min: 0,
      max: 1,
    });
