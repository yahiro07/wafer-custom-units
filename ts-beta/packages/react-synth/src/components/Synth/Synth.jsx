...
class Synth extends React.Component {
  ...
  quantizeNote = (note) => {
    if (!this.state.quantizationKey || !this.state.quantizationScale)
      return note;
    const scale = scaleOffsets[this.state.quantizationScale].map(
      (offset) => offset + this.state.quantizationKey,
    );
    let noteCheck = note;
  ...
  keydownFunction = (e) => {
    ...
    switch (e.which) {
      case 90:
        return this.octaveDown(); // Z
      case 88:
        return this.octaveUp(); // X
      case 77:
        return this.state.analyserActive
          ? this.stopAnalyser()
          : this.startAnalyser(); // M
    }
  ...
  render() {
    return (
      <ComponentContainer
        className={`${this.BASE_CLASS_NAME} ${this.props.className}`.trim()}
      >
        <Modal
        ...
        />
        <SynthContainer>
          <InfoToggle onClick={() => this.setState({ modalActive: true })}>
            i
          </InfoToggle>
          <TopBar>
            <TopSection>
            ...
            <TopSection>
              ...
              <QSelect
                onChange={(e) => {
                  this.setState({
                    quantizationKey: Number(e.target.value) || "",
                  });
                  e.target.blur();
                }}
              ...
              >
                ...
                {Object.keys(keyToNoteMap).map((key) => (
                  <option key={`keys_${key}`} value={keyToNoteMap[key]}>
                    {key}
                  </option>
                ))}
              </QSelect>
              ...
              >
                ...
                {Object.keys(scaleOffsets).map((scale) => (
                  <option key={`scales_${scale}`} value={scale}>
                    {scale}
                  </option>
                ))}
              </QSelect>
                  ...
                  this.setState({ sub1Offset: val }, () => {
                    if (this.state.noteHeld) {
                      const newFreq = getFreqFromNote(
                        this.quantizeNote(
                          this.state.noteHeld + this.state.sub1Offset,
                        ),
                      );
                ...
                onUpdate={(val) => {
                  this.setState({ sub2Offset: val }, () => {
                    if (this.state.noteHeld) {
                      const newFreq = getFreqFromNote(
                        this.quantizeNote(
                          this.state.noteHeld + this.state.sub2Offset,
                        ),
                      );
