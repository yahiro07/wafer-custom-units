<template>
  ...
  <div class="vue-audio-mixer" :class="[themeClassSize, themeClassColour, trackClass]" :style="{ width: mixerWidth }">
    ...
    <p class="vue-audio-mixer-error" v-if="track_load_error">Track {{ track_load_error }} failed to load. Check that the
      track is hosted on the same domain as the mixer, or that CORS is enabled on the track's hosting service.</p>
