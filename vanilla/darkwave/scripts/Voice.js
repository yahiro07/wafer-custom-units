...
function Voice(waveType, frequency, volume) {
  const audioContext = D.getAudioContext();
...
Object.assign(Voice.prototype, {
  setWaveType: function (input) {
    ...
    if (["sine", "square", "sawtooth", "triangle"].includes(input) !== true) {
      input = "square";
    }
  ...
  setFrequency: function (input) {
    this.oscillator.frequency.value = typeof input === "number" ? input : 220;
  },
  setVolume: function (input) {
    this.gainNode.gain.value = typeof input === "number" ? input : 0.1;
  },
  play: function () {
