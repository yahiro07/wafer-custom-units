...
function initializeInterface() {
  ...
  UI.vcfCutoffSlider.addEventListener("input", () => {
    ...
    filterCompensation.gain.linearRampToValueAtTime(
      compensation,
      Tone.now() + 0.005,
    );
  });
...
function handleRibbonMove(event) {
  ...
  if (window.ribbonMode === "bypass") {
    ...
    window.baseFrequency =
      currentFreq + (nextFreq - currentFreq) * localPosition;
  } else {
    ...
    const currentScaleNote = getClosestScaleNote(
      Math.floor(currentPosition),
      window.ribbonMode,
    );
    const nextScaleNote = getClosestScaleNote(
    ...
    window.baseFrequency =
      currentFreq + (nextFreq - currentFreq) * interpolationPosition;
  }
