...
window.oscillators = {
  vco1: {
    // VCO1 is the carrier oscillator, receiving modulation from VCO2
    instance: new Tone.Oscillator(32.7, "sawtooth"), // Start at C1
  },
  vco2: {
    // VCO2 is the modulator oscillator, with wider pitch range below center
...
window.updateOscillatorFrequencies = function () {
  ...
  oscillators.vco1.instance.frequency.linearRampToValueAtTime(
    vco1Frequency,
    Tone.now() + 0.005,
  );
  oscillators.vco2.instance.frequency.linearRampToValueAtTime(
    vco2Frequency,
    Tone.now() + 0.005,
  );
