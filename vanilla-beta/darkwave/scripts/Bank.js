...
["addVoice", "removeVoice", "setVoices"].forEach(function (name) {
  Bank.all[name] = function (input) {
    this.forEach(function (bank) {
      bank[name](input);
    });
  };
});
["WaveType", "FrequencyFactor", "VolumeFactor"].forEach(function (nameShort) {
...
Object.assign(Bank.prototype, {
  addVoice: function (param) {
    this.voices.push(new Voice(param.waveType));
    this.frequencyFactors.push(param.frequencyFactor);
    this.volumeFactors.push(param.volumeFactor);
  ...
  setVoices: function (params) {
    ...
    this.voices.forEach(Voice.destroy);
    ...
    if (params instanceof Array) {
      params.forEach(function (param) {
        that.voices.push(new Voice(param.waveType));
        that.frequencyFactors.push(param.frequencyFactor);
        that.volumeFactors.push(param.volumeFactor);
      });
    }
  ...
  setFrequency: function (input) {
    ...
    this.voices.forEach(function (voice, i) {
      voice.setFrequency(that.frequency * that.frequencyFactors[i]);
    });
  },
  setVolume: function (input) {
    ...
    this.volume = input;
    this.voices.forEach(function (voice, i) {
      voice.setVolume(that.volume * that.volumeFactors[i]);
    });
  },
