...
function Param(waveType, frequencyFactor, volumeFactor) {
  ...
  const template = document.getElementById("param"),
    clone = template.content.cloneNode(true),
    paramsTable = document.querySelector("#params table");

  paramsTable.appendChild(clone);
  this.domElement = paramsTable.querySelector("tr.param:last-child");

  this.domElement.querySelector(".param-index").innerText = this.index + 1;
  ...
  ["waveType", "frequencyFactor", "volumeFactor"].forEach(function (name) {
    param.domElement
      .querySelector(`[name=${name}]`)
      .addEventListener("change", function (event) {
        param["set" + name[0].toUpperCase() + name.substr(1)](
          event.target.value,
        );
      });
  });
...
Object.assign(Param, {
  ...
  remove: function (param) {
    ...
    Param.all.forEach(function (param, i) {
      param.index = i;
      param.domElement.querySelector(".param-index").innerText = i + 1;
    });
  ...
  updateUrlFromParams: function () {
    history.replaceState(
      ...
      "Darkwave.js",
      Param.all.reduce(function (hash, param, i) {
        return (
          hash +
          (i > 0 ? ";" : "") +
          param.waveType +
...
Object.assign(Param.prototype, {
  setWaveType: function (waveType, skipUpdate) {
    if (
      ["sine", "square", "sawtooth", "triangle"].includes(waveType) !== true
    ) {
      waveType = "square";
    }
  ...
  setFrequencyFactor: function (frequencyFactor, skipUpdate) {
    ...
    if (typeof frequencyFactor !== "number" || isNaN(frequencyFactor))
      frequencyFactor = 1;

    this.frequencyFactor = frequencyFactor;
    this.domElement.querySelector("[name=frequencyFactor]").value =
      parseFloat(frequencyFactor);
    if (skipUpdate !== true) {
  ...
  setVolumeFactor: function (volumeFactor, skipUpdate) {
    ...
    this.domElement.querySelector("[name=volumeFactor]").value =
      parseFloat(volumeFactor);
    if (skipUpdate !== true) {
