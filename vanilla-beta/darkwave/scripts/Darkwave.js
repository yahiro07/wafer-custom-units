...
function boot() {
  ...
  banks = banks.fill(0).map(function () {
...
function loop() {
  if (isListening) {
    ...
    const amplitudeMax = Array.from(timeDomainData).sort(function (a, b) {
      return a - b;
    })[0];
    ...
    const barWidth = graphCanvas.width / analyser.frequencyBinCount,
    ...
    for (let i = 0; i < analyser.frequencyBinCount; i++) {
      ...
      graphContext.fillStyle =
        "hsl( " + (i / analyser.frequencyBinCount) * 150 + ", 100%, 50% )";
      graphContext.fillRect(
        xFloor,
        graphCanvas.height - barHeight,
        barWidthCeil, //barWidth,
    ...
    if (isPlaying) {
      ...
      const processed = frequencyData
        ...
        .forEach(function (item, i) {
          const isGood =
            item.volume > 0.4 && isFinite(item.noteNumber) && item.frequency > 0
              ? true
              : false;
    ...
    banks.forEach(function (bank, i) {
      bank.domElement.classList[bank.isActive ? "add" : "remove"]("active");
      bank.domElement.style.left =
        (bank.frequency / audioContext.sampleRate) * 100 + "%";
...
function pause() {
  isPlaying = false;
  Bank.all.forEach(function (bank) {
    bank.isActive = false;
    bank.setVolume(0);
  });
}
...
function peakClassifier(frames, peakThreshold) {
  ...
  for (let i = 0; i < frames.length; i++) {
    ...
    for (let bin = kernelWidth; bin < fft.length - kernelWidth; bin++) {
      const kernelAvgL = 0,
        kernelAvgR = 0,
        kernelSum = 0;
...
function peakClassifierMax(frms, peakThresh) {
  ...
  for (let t = 0; t < frms.length; t++) {
    ...
    for (let bin = kernelWidth; bin < fft.length - kernelWidth; bin++) {
      const kernelMaxL = -1000, //  init arbitrarily min number.
...
function dataFromFrequency(frequency) {
  const noteNumber = getNoteNumberFromFrequencyHz(frequency),
...
function getDetuneCents(frequencyHz, noteNumber) {
  return Math.floor(
    (1200 * Math.log(frequencyHz / getFrequencyHzFromNoteNumber(noteNumber))) /
      Math.log(2),
  );
}
