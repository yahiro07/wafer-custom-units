...
function changeEffect() {
  ...
  switch (effect) {
    ...
    case 12: // Ping-pong delay
      var pingPong = createPingPongDelay(
        audioContext,
        audioInput == realAudioInput,
        0.3,
        0.4,
...
function createDelay() {
  ...
  delayNode.delayTime.value = parseFloat(
...
function createDistortion() {
  if (!waveshaper) waveshaper = new WaveShaper(audioContext);
...
function createFilterLFO() {
  ...
  gain.gain.value =
    2500 * parseFloat(document.getElementById("lplfodepth").value);
...
function createRingmod() {
  ...
  osc.type = "sine";
  rmod = osc;
  osc.frequency.value = Math.pow(
    2,
    parseFloat(document.getElementById("rmfreq").value),
  );
...
function createFlange() {
  var delayNode = audioContext.createDelay();
  delayNode.delayTime.value = parseFloat(
    document.getElementById("fldelay").value,
  );
  fldelay = delayNode;
...
function createStereoChorus() {
  ...
  delayLNode.delayTime.value = parseFloat(
    document.getElementById("scdelay").value,
  );
  delayRNode.delayTime.value = parseFloat(
    document.getElementById("scdelay").value,
  );
  scldelay = delayLNode;
  scrdelay = delayRNode;
...
function createModDelay() {
  ...
  var delayNode = audioContext.createDelay();
  delayNode.delayTime.value = parseFloat(
    document.getElementById("mdtime").value,
  );
  mdtime = delayNode;

  var feedbackGainNode = audioContext.createGain();
  feedbackGainNode.gain.value = parseFloat(
    document.getElementById("mdfeedback").value,
  );
  ...
  chorusRateGainNode.gain.value = parseFloat(
    document.getElementById("mddepth").value,
...
function createStereoFlange() {
  ...
  sfllfb.gain.value = sflrfb.gain.value = parseFloat(
    document.getElementById("sflfb").value,
  );

  inputNode.connect(splitter);
  inputNode.connect(wetGain);

  sflldelay.delayTime.value = parseFloat(
    document.getElementById("sfldelay").value,
  );
  sflrdelay.delayTime.value = parseFloat(
    document.getElementById("sfldelay").value,
  );
  ...
  sflspeed.frequency.value = parseFloat(
    document.getElementById("sflspeed").value,
  );
...
function createNoiseGate() {
  ...
  ngGate.curve = generateNoiseFloorCurve(
    parseFloat(document.getElementById("ngFloor").value),
  );
...
function setBitCrusherDepth(bits) {
  ...
  console.log(
    "setting bitcrusher depth to " + bits + " bits, length = " + length,
  );
  var curve = new Float32Array(length);

  var lengthMinusOne = length - 1;

  for (var i = 0; i < length; i++) curve[i] = (2 * i) / lengthMinusOne - 1;
