...
class Knob extends React.Component {
  ...
  getKnobRotationFromValue = (value) => {
    ...
    return type === 1
      ? ((value - offset) / modifier) * 2 * this.maxKnobRotation -
          this.maxKnobRotation
  ...
  updateRotation = (mouseY) => {
    ...
    if (newRotation > this.maxKnobRotation) {
      newRotation = this.maxKnobRotation;
    } else if (newRotation < -this.maxKnobRotation) {
      newRotation = -this.maxKnobRotation;
    }
  ...
  getDashOffset = (val, type) => {
    return (
      ...
      (184 / (this.maxKnobRotation * 2)) * (val + this.typeValueOffsets[type])
    );
  };
  ...
  render = () => {
    ...
    return (
      <ComponentContainer className={`${this.BASE_CLASS_NAME}`}>
        ...
        >
          <KnobSvg
            className={`${this.BASE_CLASS_NAME}__svg`}
            viewBox="0 0 100 100"
          >
            <path d={"M20,76 A 40 40 0 1 1 80 76"} stroke={knobTrackSvgColor} />
            <path
        ...
        <Label>
          ...
          <div className="value-text">
            {Math.round(
              this.getValueFromKnobRotation(this.state.rotation) * 100,
            ) / 100}
          </div>
        </Label>
      </ComponentContainer>
