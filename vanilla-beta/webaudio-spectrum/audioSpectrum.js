...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const oscilloscopeScaleBtn = document.getElementById(
    "toggleOscilloscopeScale",
  );
  ...
  const oscilloscopeContainer = document.getElementById(
  ...
  function computeSpectrumBytes(useWindow) {
    ...
    if (useWindow && hammingTimeWeights && hammingTimeWeights.length === n) {
      for (let i = 0; i < n; i++)
        fftWorkRe[i] = time[i] * hammingTimeWeights[i];
    } else {
    ...
    for (let k = 0; k < half; k++) {
      customSmoothPrev[k] = Math.round(
        CUSTOM_SMOOTH_ALPHA * customSmoothPrev[k] +
          (1 - CUSTOM_SMOOTH_ALPHA) * out[k],
      );
  ...
  function drawFrequencyAxis(
    ...
    if (phase === "grid") {
      ...
      for (let i = 0; i < freqs.length; i++) {
        const x =
          Math.round(frequencyToX(freqs[i], width, sampleRate, mode)) + 0.5;
        context.moveTo(x, 0);
      ...
      if (mode === "log") {
        for (let i = 0; i < minor.length; i++) {
          const x =
            Math.round(frequencyToX(minor[i], width, sampleRate, mode)) + 0.5;
          context.moveTo(x, 0);
          context.lineTo(x, height);
        }
      }
  ...
  function drawOscilloscope() {
    ...
    for (let i = 0; i < samplesToShow; i++) {
      ...
      if (oscilloscopeScaleMode === "log") {
        ...
        const scaled =
          (sign * (1 - Math.exp(-3 * abs))) / (1 + Math.exp(-3 * (abs - 0.5)));
        v = 1 + scaled;
      }
