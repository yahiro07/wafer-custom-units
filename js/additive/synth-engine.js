...
const PRESETS = {
  ...
  "Pulse 25%": {
    ...
    harmonics: Array.from(
      { length: 32 },
      (_, i) => Math.abs(Math.sin(Math.PI * (i + 1) * 0.25)) / (i + 1),
    ),
  ...
  "Pulse 10%": {
    ...
    harmonics: Array.from(
      { length: 32 },
      (_, i) => Math.abs(Math.sin(Math.PI * (i + 1) * 0.1)) / (i + 1),
    ),
    adsr: { attack: 0.01, decay: 0.15, sustain: 0.7, release: 0.3 },
  },
  ...
  "Formant Sweep": {
    ...
    harmonics: Array.from(
      { length: 32 },
      (_, i) => Math.exp(-((i - 8) * (i - 8)) / 20) * 0.8 + (i === 0 ? 0.5 : 0),
    ),
