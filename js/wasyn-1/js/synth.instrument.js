  ...
  this.presets = [
    {
      name: "Basic",
      value: [
        0, 64, 64, 0, 0, 64, 64, 0, 127, 0, 0, 64, 64, 0, 64, 64, 0, 16, 0, 0,
        0, 16, 0, 0, 3,
      ],
    },
    {
      name: "Night Ride",
      value: [
        0, 64, 64, 14, 0, 64, 64, 7, 113, 32, 0, 64, 77, 0, 64, 64, 0, 16, 0, 0,
        0, 16, 0, 0, 3,
      ],
    },
    {
      name: "Crystal Caves",
      value: [
        0, 64, 64, 9, 0, 0, 60, 27, 127, 0, 0, 127, 71, 1, 127, 64, 1, 16, 0, 0,
        0, 53, 1, 0, 2,
      ],
    },
    {
      name: "Old VHS Tape",
      value: [
        0, 64, 64, 55, 0, 33, 10, 30, 68, 67, 0, 64, 64, 1, 65, 70, 1, 4, 7, 3,
        3, 6, 1, 3, 2,
      ],
    },
    {
      name: "Sunburst Pad",
      value: [
        20, 64, 64, 52, 26, 42, 0, 52, 64, 51, 0, 64, 64, 0, 108, 69, 1, 16, 0,
        0, 0, 16, 0, 0, 3,
      ],
    },
    {
      name: "Cascade Pad",
      value: [
        127, 64, 64, 70, 0, 80, 0, 102, 64, 59, 0, 108, 72, 0, 108, 65, 0, 16,
        0, 0, 0, 14, 9, 1, 3,
      ],
    },
    {
      name: "Rave Sound",
      value: [
        0, 4, 0, 0, 0, 64, 64, 5, 88, 0, 0, 108, 63, 0, 113, 64, 0, 16, 0, 0, 0,
        16, 0, 0, 3,
      ],
    },
    {
      name: "Fatalogue",
      value: [
        0, 64, 64, 14, 4, 18, 0, 12, 49, 61, 0, 18, 83, 0, 19, 64, 0, 16, 0, 0,
        0, 16, 0, 0, 3,
      ],
    },
    {
      name: "Hazy Morning",
      value: [
        0, 64, 64, 18, 0, 113, 72, 127, 56, 99, 0, 64, 65, 0, 108, 69, 1, 5, 14,
        3, 3, 27, 1, 3, 0,
      ],
    },
    {
      name: "Wub One",
      value: [
        0, 52, 66, 3, 25, 94, 87, 0, 76, 127, 0, 0, 61, 1, 18, 80, 0, 0, 0, 2,
        3, 18, 127, 3, 3,
      ],
    },
    {
      name: "Freeze Ray FX",
      value: [
        6, 64, 64, 17, 22, 63, 63, 17, 127, 53, 0, 127, 127, 0, 127, 64, 1, 16,
        63, 0, 0, 101, 6, 3, 2,
      ],
    },
  ];
...
synth.prototype = {
  ...
  setControlValue: function (id, value) {
    ...
    switch (id) {
      ...
      case 8:
        ...
        for (var i = 0; i < this.polyphony; i++) {
          this.filterNodes[i].frequency.setValueAtTime(
            value,
  ...
  noteOn: function (noteNumber, time, velocity) {
    ...
    for (var i = 0; i < this.oscsPerVoice; i++) {
      ...
      oscNode.frequency.setValueAtTime(
        frequency,
        when + this.timePadding,
      );
    }

    //Start the envelopes
    this.ampEnvelopeStart(currentVoice, when);
    this.filterEnvelopeStart(currentVoice, when);
  },

  //-------

  noteOff: function (noteNumber, time) {
    ...
    //Envelope end
    this.ampEnvelopeEnd(voice, when);
    this.filterEnvelopeEnd(voice, when);
  ...
  ampEnvelopeStart: function (voice, when) {
    ...
    ampNode.gain.cancelScheduledValues(when);
    ampNode.gain.setValueAtTime(ampNode.gain.value, when);
    ampNode.gain.linearRampToValueAtTime(0, when + this.timePadding);

    //Attack phase
    var attackTime = this.timePadding + this.ampEnv.attack;
    ampNode.gain.linearRampToValueAtTime(1, when + attackTime);
  ...
  ampEnvelopeEnd: function (voice, when) {
    when = when ?? this.context.currentTime;
    var ampNode = this.ampNodes[voice];

    //Release phase
    ampNode.gain.cancelScheduledValues(when);
    ampNode.gain.cancelAndHoldAtTime(when);
    ampNode.gain.setTargetAtTime(
      0,
      when,
      this.timePadding + this.ampEnv.release,
    );
  },

  //-------

  filterEnvelopeStart: function (voice, when) {
    when = when ?? this.context.currentTime;
    ...
    filterNode.detune.cancelScheduledValues(when);
    filterNode.detune.setValueAtTime(filterNode.detune.value, when);
    //filterNode.detune.linearRampToValueAtTime(1, when + this.timePadding);
    ...
    filterNode.detune.linearRampToValueAtTime(
      targetFrequency,
      when + attackTime,
    );
  ...
  filterEnvelopeEnd: function (voice, when) {
    when = when ?? this.context.currentTime;
    var filterNode = this.filterNodes[voice];

    //Release phase
    filterNode.detune.cancelScheduledValues(when);
    filterNode.detune.cancelAndHoldAtTime(when);
    filterNode.detune.setTargetAtTime(
      this.filtCutoffFrequency,
      when,
      this.timePadding + this.filtEnv.release,
    );
  },
  ...
  setOscType: function (osc, value) {
    var options = ["sawtooth", "square", "triangle", "sine"];
  ...
  setLfoRate: function (lfoID, value) {
    ...
    this.lfoNodes[lfoID].frequency.setValueAtTime(
      value,
      this.context.currentTime,
    );
  },
  ...
  setLfoAmount: function (lfoID, value) {
    ...
    this.lfoGainNodes[lfoID].gain.setValueAtTime(
      value,
      this.context.currentTime,
    );
  },
  ...
  animate: function () {
    ...
    for (var key in this.ampNodes) {
      ...
      $('.js-voice-filter[data-id="' + key + '"] div').height(
        filtPercent + "%",
      );
    }
  },
