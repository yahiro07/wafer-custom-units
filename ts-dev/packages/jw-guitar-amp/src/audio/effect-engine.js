...
export function setupEffectEngine() {
  ...
  const setupConnectionChain = (source, destination) => {
    source
      .connect(trebleEQ)
      .connect(midEQ)
      .connect(bassEQ)
  ...
  const setupContext = async () => {
    ...
    setupConnectionChain(source, context.destination);
  };

  const setupUnitSignals = () => {
    setupConnectionChain(
      unitInterface.audioInputNode,
